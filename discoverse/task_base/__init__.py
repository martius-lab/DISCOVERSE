...
def playback_and_render(data_path, output_dir, randomize_visuals=False):
    """Playback recorded states and render observations"""
    ...
    for cam_id in cfg.obs_rgb_cam_id:
        frames = [o['img'][cam_id] for o in obs_lst]
        output_path = os.path.join(output_dir, f"cam_{{cam_id}}_playback.mp4")
        mediapy.write_video(output_path, frames, fps=cfg.render_set["fps"])
        print(f"Saved video: {{output_path}}")

    # Save depth if available
    if hasattr(obs_lst[0], 'depth') and obs_lst[0]['depth']:
        for cam_id in cfg.obs_depth_cam_id:
            depth_frames = [o['depth'][cam_id] for o in obs_lst]
            # Convert depth to visualizable format
            import cv2
            depth_vis = []
            for d in depth_frames:
                d_vis = cv2.applyColorMap(
                    cv2.convertScaleAbs(d, alpha=255./5.0),
                    cv2.COLORMAP_JET
                )
                depth_vis.append(d_vis)
            output_path = os.path.join(output_dir, f"cam_{{cam_id}}_depth_playback.mp4")
            mediapy.write_video(output_path, depth_vis, fps=cfg.render_set["fps"])
...
if __name__ == "__main__":
    ...
    for traj_path in traj_dirs:
        ...
        output_dir = os.path.join(traj_path, "rendered")
        playback_and_render(traj_path, output_dir, args.randomize_visuals)
