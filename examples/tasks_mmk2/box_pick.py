...
cfg.mjcf_file_path = "mjcf/tasks_mmk2/box_pick.xml"
cfg.obj_list = ["box"]
cfg.sync = True
cfg.headless = False
...
cfg.init_state["base_position"] = [0.3, 0.25, 0.0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_gs", action="store_true", help="Use gaussian splatting renderer"
    )
    parser.add_argument(
        "--render",
        action="store_true",
        help="Render observations after simulation (runs record_playback.py)",
    )
    ...
    print(f"Initial base position: {sim_node.mj_data.qpos[:3]}")
    print(f"Initial base orientation: {sim_node.mj_data.qpos[3:7]}")
