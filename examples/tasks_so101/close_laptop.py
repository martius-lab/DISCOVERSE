...
class SimNode(SO101TaskBase):
    ...
    def check_success(self):
        return (abs(self.mj_data.qpos[7]) < 0.1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_set_size", type=int, default=2, help="data set size")
    ...
    # convert trmat_01 (rotation matrix) to quaternion in MuJoCo order [w, x, y, z]
    trmat_01 = Rotation.from_matrix(trmat_01).as_quat()[[3, 0, 1, 2]]  
    trmat_2 = Rotation.from_matrix(trmat_2).as_quat()[[3, 0, 1, 2]]  
    ...
    while sim_node.running:
        ...
        try:
            if stm.trigger():
                if stm.state_idx == 0: # 伸到电脑屏幕上方
                    ...
                    sim_node.target_control[:5] = arm_ik.solve_ik(tmat_tgt_local[:3,3], trmat_01, sim_node.mj_data.qpos[:5])
                    print("Move to laptop screen top")
                    sim_node.target_control[5] = 0.75
                    ...
                    sim_node.target_control[:5] = arm_ik.solve_ik(tmat_tgt_local[:3,3], trmat_2, sim_node.mj_data.qpos[:5])
        ...
        action[5] = sim_node.target_control[5]

        obs, _, _, _, _ = sim_node.step(action)
        try:
            if len(obs_lst) < sim_node.mj_data.time * cfg.render_set["fps"]:
                print(f"Recording frame at time {obs['time']:.3f}s, total frames: {len(obs_lst)+1}")
                imgs = obs.pop("img")
